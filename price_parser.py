...
async def get_page_data(session,page):
    ...
    url = f'https://mosplitka.ru/catalog/plitka/view_product/?PAGEN_1={page}'
    print(f'[INFO] - начал обрабатывать страницу {page}')
    
    try:
        response = await session.get(url=url,  headers=headers, timeout=4000)
    except:
    ...
    async with aiohttp.ClientSession(trust_env=True, timeout=4000) as session:
        ...
        try:
            cards: ResultSet = soup.find('div', class_="products").find_all('div', class_='m-product')        
        ...
        for card in cards: 
                try:
                    articul = card.find('div', class_='article--wrapper').text[5::]
                except AttributeError:
                    articul = ''

                try:
                    new_price = card.find('span', class_='cost_value').text
                except AttributeError:
                    new_price = ''
                try:
                    new_price_q = card.find('span', class_='cost_currency').text
                except AttributeError:
                    new_price_q = ''
                ...
                if articul != '':
                    ...
                    second_url = f'https://diler.mosplitka.ru/catalog/?login=yes&q={articul}'
                    # print(second_url)
                    second_response = await session.post(url=second_url, data=data,headers=headers, timeout=4000)
                    second_soup = BeautifulSoup(await second_response.text(), "lxml")
                    card = second_soup.find('div', class_="catalog__inner-container catalog__inner-container--content").find('div', class_='catalog__result-item')
                    # print(card)
                    ...
                    if card != None:
                        ...
                        if obj['in_stock_krasnodar'] == '0' and obj['in_stock_podolsk'] == '0':
                            print(f'[PRODUCT INFO] - нету остатков {articul}')
                        else:
                            result.append(obj)
...
OUT_XLSX_FILENAME = f'price_{cur_time}.xlsx'
    ...
    with xlsxwriter.Workbook(file_name) as workbook:
        ...
        for col, h in enumerate(headers):
            ...
            for row, item in enumerate(data, start=1):
                ...
                ws.write_string(row, 19, item['new_price'])
                ws.write_string(row, 20, item['new_price_q'])



async def gather_data():

    url = f'https://mosplitka.ru/catalog/plitka/view_product'

    async with aiohttp.ClientSession(trust_env=True) as session:
        response = await session.get(url=url,  headers=headers)
        soup = BeautifulSoup(await response.text(), "lxml")
        pages_count = int(soup.find('ul', class_='pagination-catalog__items').find_all('li')[-1].text)
        tasks = []
        # print(pages_count)
